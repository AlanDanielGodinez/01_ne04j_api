...
// Transferir todos los activos de una empresa a otra
MATCH (emOri:Empresa {RFC: "4k9lPqR6yH0nJ7bV3x"})-[r]->(asset) // Encuentra todas las relaciones y nodos conectados a la empresa origen
MATCH (emDes:Empresa {RFC: "5r4eG6h7jU8i9o0pL"})

FOREACH (_ IN CASE WHEN type(r) = 'PERSONAL' THEN [1] ELSE [] END | MERGE (emDes)-[:PERSONAL]->(asset))
FOREACH (_ IN CASE WHEN type(r) = 'AVIONES' THEN [1] ELSE [] END | MERGE (emDes)-[:AVIONES]->(asset))
FOREACH (_ IN CASE WHEN type(r) = 'VUELOS' THEN [1] ELSE [] END | MERGE (emDes)-[:VUELOS]->(asset))
// Eliminar relaciones en la empresa origen
DELETE r;
